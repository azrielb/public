...
namespace AzrielClasses
{
    public static class checkings
    {
        ...
        public static bool EmailAddressIsValid(string email, string regex = null)
        {
            if (regex == null) regex = @"^[\w\-\._]+\@\w[\w\-\.]*\.\w+$";
            return email != null && new Regex(regex).IsMatch(email);
        }
        public static bool IsValidEmailAddress(this string email, string regex = null)
        {
            return EmailAddressIsValid(email, regex);
        }
    }
}
